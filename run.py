...
def get_args():
    ...
    """
    parser = argparse.ArgumentParser(description="GNet for base resolution signal prediction and binding sequence determination.")
...
def main():
    ...
    motifLen = 16
    Data1 = np.load(osp.join(args.data_dir, '%s_train.npz' % args.name))
    seqs1, signals = Data1['data'], Data1['signal']
    Data2 = np.load(osp.join(args.data_dir, '%s4_train.npz' % args.name))
    seqs2 = Data2['data']
