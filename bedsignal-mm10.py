...
SEQ_LEN = 1001
OFFSET = 3000
INDEX = ['chr' + str(i + 1) for i in range(20)]
INDEX[19] = 'chrX'
SMOOTH = True
WINDOW = 15
THRESHOLD = 0.05
...
CHROM = {}
with open(ROOT + './Genome/chromsize1') as f:
    for i in f:
...
def main():
    ...
    if not osp.exists(out_dir):
        os.mkdir(out_dir)
    sequence_dict = SeqIO.to_dict(SeqIO.parse(open(ROOT + './Genome/mm10.fa'), 'fasta'))
    ...
    seqs_total = np.concatenate((seqs_tr, seqs_te, seqs_neg_tr, seqs_neg_te), axis=0)
    signals_total = np.concatenate((signals_tr, signals_te, signals_neg_tr, signals_neg_te))
    labels_total = np.concatenate((labels_tr, labels_te, labels_neg_tr, labels_neg_te), axis=0)
